...
with col_izq:
    ...
    with st.form("input_form"):
        lead_time = st.number_input("Días de antelación", 0, 737, 30,
                                    help="Días entre la reserva y la llegada", key="lead_time")
        adults = st.number_input("Número de adultos", 1, 10, 2, key="adults")
        adr = st.number_input("Precio por noche (€)", 0.0, 5000.0, 100.0, key="adr")
        prev_cancellations = st.number_input("Cancelaciones previas del cliente", 0, 50, 0, key="prev_cancel")
        special_requests = st.number_input("Peticiones especiales", 0, 10, 0, key="special_req")
        deposit = st.selectbox("Tipo de depósito",
                               ["No Deposit", "Non Refund", "Refundable"], key="deposit")
        hotel = st.selectbox("Tipo de hotel",
                             ["City Hotel", "Resort Hotel"], key="hotel")
...
with col_der:
    ...
    if submitted:
        ...
        # --- Perfil base: reserva típica del dataset (mediana/moda) ---
        # Sin esto, las 238 variables restantes quedan a 0 y el escalado
        # las distorsiona, haciendo que lead_time no influya en la predicción
        input_dict['arrival_date_year'] = 2016
        input_dict['stays_in_weekend_nights'] = 1
        input_dict['stays_in_week_nights'] = 2
        input_dict['arrival_month_num'] = 7
        input_dict['meal_BB'] = 1
        input_dict['country_PRT'] = 1
        input_dict['market_segment_Direct'] = 1
        input_dict['distribution_channel_Direct'] = 1
        input_dict['reserved_room_type_A'] = 1
        input_dict['assigned_room_type_A'] = 1
        input_dict['customer_type_Transient'] = 1
        input_dict['total_nights'] = 3
        ...
        input_dict['total_guests'] = adults
        input_dict['revenue_estimate'] = adr * 3
        ...
        try:
            ...
            fig = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    'bar': {'color': '#1B4332'},
                    'steps': [
                        {'range': [0, 25], 'color': '#B7E4C7'},
                        {'range': [25, 40], 'color': '#FFE082'},
                        {'range': [40, 100], 'color': '#EF9A9A'}
                    ],
                    'threshold': {
...
if st.button("Limpiar y reiniciar"):
    # Borramos las keys del formulario del estado de la sesión
    # para que los campos vuelvan a sus valores por defecto
    for k in ["lead_time", "adults", "adr", "prev_cancel",
              "special_req", "deposit", "hotel"]:
